...
public class LegadoConfiguracao extends AbstratoConfiguracao {
	...
	public LegadoConfiguracao(Formulario formulario) {
		super(formulario, LegadoMensagens.getString("label.plugin_legado"));
		montarLayout();
		configurar();
	}

	private void montarLayout() {
		PanelCenter panelPosicoes = criarPainelGrupo(posicoes, LegadoPreferencia.getLegadoPosicaoAbaFichario());
		chkExibirArqIgnorados.setSelected(LegadoPreferencia.isExibirArqIgnorados());
	...
	private void configurar() {
		chkExibirArqIgnorados
				.addActionListener(e -> LegadoPreferencia.setExibirArqIgnorados(chkExibirArqIgnorados.isSelected()));
	}

	private Label criarLabelTituloRotulo(String rotulo) {
		return criarLabelTitulo(LegadoMensagens.getString(rotulo), false);
	}
	...
	public Label criarLabel(String chaveRotulo) {
		return new Label(LegadoMensagens.getString(chaveRotulo), false);
	}

	static CheckBox criarCheckBox(String chaveRotulo) {
		return new CheckBox(LegadoMensagens.getString(chaveRotulo), false);
	}
		...
		private RadioPosicao(NomeValor nomeValor) {
			...
			addActionListener(e -> {
				if (nomeValor.tipo == NomeValor.POSICAO_ABA) {
					LegadoPreferencia.setLegadoPosicaoAbaFichario(nomeValor.valor);
				}
			});
		}
	}
