...
public class ExpressaoModelo extends AbstractTableModel implements BuscaConteudo {
	private static final String[] COLUNAS = { "NOME", "VALOR" };
	private static final long serialVersionUID = 1L;

	@Override
	public int getRowCount() {
		return ExpressaoProvedor.getSize();
	}
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		Expressao item = ExpressaoProvedor.getExpressao(rowIndex);
		if (columnIndex == 0) {
			return item.getNome();
	...
	public void contemConteudo(Set<String> set, String string, boolean porParte) {
		ExpressaoProvedor.contemConteudo(set, string, porParte);
	}
}
