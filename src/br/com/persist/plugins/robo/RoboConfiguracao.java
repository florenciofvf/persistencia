...
public class RoboConfiguracao extends AbstratoConfiguracao {
	...
	public RoboConfiguracao(Formulario formulario) {
		super(formulario, RoboMensagens.getString("label.plugin_robo"));
		montarLayout();
		configurar();
	}

	private void montarLayout() {
		PanelCenter panelPosicoes = criarPainelGrupo(posicoes, RoboPreferencia.getRoboPosicaoAbaFichario());
		chkExibirArqIgnorados.setSelected(RoboPreferencia.isExibirArqIgnorados());
	...
	private void configurar() {
		chkExibirArqIgnorados
				.addActionListener(e -> RoboPreferencia.setExibirArqIgnorados(chkExibirArqIgnorados.isSelected()));
	}

	private Label criarLabelTituloRotulo(String rotulo) {
		return criarLabelTitulo(RoboMensagens.getString(rotulo), false);
	}
	...
	public Label criarLabel(String chaveRotulo) {
		return new Label(RoboMensagens.getString(chaveRotulo), false);
	}

	static CheckBox criarCheckBox(String chaveRotulo) {
		return new CheckBox(RoboMensagens.getString(chaveRotulo), false);
	}
		...
		private RadioPosicao(NomeValor nomeValor) {
			...
			addActionListener(e -> {
				if (nomeValor.tipo == NomeValor.POSICAO_ABA) {
					RoboPreferencia.setRoboPosicaoAbaFichario(nomeValor.valor);
				}
			});
		}
	}
