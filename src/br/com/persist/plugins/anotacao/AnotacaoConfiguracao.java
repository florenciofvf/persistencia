...
public class AnotacaoConfiguracao extends AbstratoConfiguracao {
	...
	public AnotacaoConfiguracao(Formulario formulario) {
		super(formulario, AnotacaoMensagens.getString("label.plugin_anotacao"));
		montarLayout();
		configurar();
	}

	private void montarLayout() {
		PanelCenter panelPosicoes = criarPainelGrupo(posicoes, AnotacaoPreferencia.getAnotacaoPosicaoAbaFichario());
		chkExibirArqIgnorados.setSelected(AnotacaoPreferencia.isExibirArqIgnorados());
	...
	private void configurar() {
		chkExibirArqIgnorados
				.addActionListener(e -> AnotacaoPreferencia.setExibirArqIgnorados(chkExibirArqIgnorados.isSelected()));
	}

	private Label criarLabelTituloRotulo(String rotulo) {
		return criarLabelTitulo(AnotacaoMensagens.getString(rotulo), false);
	}
	...
	public Label criarLabel(String chaveRotulo) {
		return new Label(AnotacaoMensagens.getString(chaveRotulo), false);
	}

	static CheckBox criarCheckBox(String chaveRotulo) {
		return new CheckBox(AnotacaoMensagens.getString(chaveRotulo), false);
	}
		...
		private RadioPosicao(NomeValor nomeValor) {
			...
			addActionListener(e -> {
				if (nomeValor.tipo == NomeValor.POSICAO_ABA) {
					AnotacaoPreferencia.setAnotacaoPosicaoAbaFichario(nomeValor.valor);
				}
			});
		}
	}
