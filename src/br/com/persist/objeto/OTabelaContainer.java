...
public class OTabelaContainer extends Panel implements ActionListener, ItemListener, Runnable, IIni {
	private static final long serialVersionUID = 1L;
	private transient OTabelaListener.ConfigAlturaAutomatica configAlturaAutomaticaListener;
	private final transient ActionListenerInner actionListenerInner = new ActionListenerInner();
	private transient OTabelaListener.BuscaAutomaticaApos buscaAutomaticaAposListener;
	private transient OTabelaListener.BuscaAutomatica buscaAutomaticaListener;
	private transient OTabelaListener.LinkAutomatico linkAutomaticoListener;
	private final Button btnArrasto = new Button(Action.actionIconDestacar());
	private transient OTabelaListener.Componente componenteListener;
	private transient OTabelaListener.Dimensao dimensaoListener;
	private transient OTabelaListener.Selecao selecaoListener;
	private transient OTabelaListener.Apelido apelidoListener;
	private transient OTabelaListener.Titulo tituloListener;
	...
	public OTabelaContainer(IJanela janela, ConexaoProvedor provedor, Conexao padrao, Objeto objeto, Graphics g,
			boolean buscaAuto) {
	...
	private class Toolbar extends BarraButton {
		...
		private class ExcluirRegistrosAcao extends Acao {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (linhas != null && linhas.length > 0 && Util.confirmaExclusao(OTabelaContainer.this, false)) {
				...
				itemAtualizarAuto.addMouseListener(new MouseAdapter() {
					...
					public void mouseEntered(MouseEvent e) {
						if (thread == null) {
							...
							thread = new Thread(OTabelaContainer.this);
							contadorAuto = 0;
							thread.start();
						}
					}
		...
		private class ButtonBuscaAuto extends ButtonPopup {
			...
			private class MenuBuscaAuto extends MenuPadrao2 {
				...
				private void processar(boolean apostrofes) {
					...
					if (lista.isEmpty()) {
						Util.mensagem(OTabelaContainer.this, grupo.getCampo() + " vazio.");
						return;
					}
					...
					if (grupo.isProcessado() && buscaAutomaticaAposListener != null) {
						buscaAutomaticaAposListener.buscaAutomaticaApos(OTabelaContainer.this, grupoApos,
					...
					TabelaUtil.ajustar(tabela, OTabelaContainer.this.getGraphics());
				}
			}
		}
				...
				dadosAcao.setActionListener(e -> {
					...
					if (model instanceof RegistroModelo) {
						...
						if (linhas != null && linhas.length == 1) {
							...
							Util.mensagem(OTabelaContainer.this, sb.toString());
						}
					}
				});
			}
			...
			private class TotalizarRegistrosAcao extends Action {
				...
				public void actionPerformed(ActionEvent e) {
					...
					try {
						...
						Util.stackTraceAndMessage("TOTAL", ex, OTabelaContainer.this);
					}
				}
			}
		}
		...
		private class ButtonInfo extends ButtonPopup {
			...
			private class ChavesExportadasAcao extends Action {
				...
				public void actionPerformed(ActionEvent e) {
					...
					try {
						...
						Util.stackTraceAndMessage("CHAVES-EXPORTADAS", ex, OTabelaContainer.this);
					}
				}
			}
			...
			private class InfoBancoAcao extends Action {
				...
				public void actionPerformed(ActionEvent e) {
					...
					try {
						...
						Util.stackTraceAndMessage("INFO-BANCO", ex, OTabelaContainer.this);
					}
				}
			}
			...
			private void configCabecalhoColuna(ListagemModelo modelo) {
				...
				for (int i = 0; i < colunas.size(); i++) {
					...
					CabecalhoColuna cabecalhoColuna = new CabecalhoColuna(OTabelaContainer.this, modeloOrdenacao,
							coluna, false);
	...
	private boolean continuar(String complemento) {
		...
		return Util.confirmar(OTabelaContainer.this, msg, false);
	}
	...
	private class ActionListenerInner implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			OTabelaContainer.this.actionPerformed(null);
	...
	public OTabelaListener.BuscaAutomatica getBuscaAutomaticaListener() {
		return buscaAutomaticaListener;
	}

	public void setBuscaAutomaticaListener(OTabelaListener.BuscaAutomatica buscaAutomaticaListener) {
		this.buscaAutomaticaListener = buscaAutomaticaListener;
	}

	public OTabelaListener.LinkAutomatico getLinkAutomaticoListener() {
		return linkAutomaticoListener;
	}

	public void setLinkAutomaticoListener(OTabelaListener.LinkAutomatico linkAutomaticoListener) {
		this.linkAutomaticoListener = linkAutomaticoListener;
	}

	public OTabelaListener.BuscaAutomaticaApos getBuscaAutomaticaAposListener() {
		return buscaAutomaticaAposListener;
	}

	public void setBuscaAutomaticaAposListener(OTabelaListener.BuscaAutomaticaApos buscaAutomaticaAposListener) {
		this.buscaAutomaticaAposListener = buscaAutomaticaAposListener;
	}

	public OTabelaListener.ConfigAlturaAutomatica getConfigAlturaAutomaticaListener() {
		return configAlturaAutomaticaListener;
	}

	public void setConfigAlturaAutomaticaListener(
			OTabelaListener.ConfigAlturaAutomatica configAlturaAutomaticaListener) {
		this.configAlturaAutomaticaListener = configAlturaAutomaticaListener;
	}

	public OTabelaListener.Titulo getTituloListener() {
		return tituloListener;
	}

	public void setTituloListener(OTabelaListener.Titulo tituloListener) {
	...
	public OTabelaListener.Selecao getSelecaoListener() {
		return selecaoListener;
	}

	public void setSelecaoListener(OTabelaListener.Selecao selecaoListener) {
		this.selecaoListener = selecaoListener;
	}

	public OTabelaListener.Dimensao getDimensaoListener() {
		return dimensaoListener;
	}

	public void setDimensaoListener(OTabelaListener.Dimensao dimensaoListener) {
	...
	public void setApelidoListener(OTabelaListener.Apelido apelidoListener) {
		this.apelidoListener = apelidoListener;
	}

	public OTabelaListener.Componente getComponenteListener() {
		return componenteListener;
	}

	public void setComponenteListener(OTabelaListener.Componente componenteListener) {
